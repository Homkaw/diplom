...
class FinanceApp(MDApp):
    ...
    def generate_report(self):
        try:
            ...
            email = screen.ids.email_field.text.strip()

            if not email or "@" not in email:
                self.show_dialog("Ошибка", "Введите корректный email.")
                return

            user = self.profile

            def get_safe(key, default="не указано"):
            ...
            report = "📄 Отчёт о пользователе\n\n"
            report += f"ФИО: {get_safe('last_name')} {get_safe('first_name')} {get_safe('middle_name')}\n"
            report += f"Школа: {get_safe('school')}, Класс: {get_safe('grade')}, Возраст: {get_safe('age')}\n"
            report += f"Баланс кешиков: {get_safe('currency', 0)}\n\n"
            report += "Прогресс по модулям:\n"

            for key, data in self.profile.get("progress", {}).items():
                progress = data.get("progress", 0)
                report += f"— {key}: {progress}%\n"

            with open("report.txt", "w", encoding="utf-8") as f:
                f.write(report)

            print("[DEBUG] Отчёт сформирован и сохранён")
            self.show_dialog("Отчёт", f"Отчёт сформирован. Вы можете отправить его вручную на {email}.")

        except Exception as e:
            print("[Ошибка генерации отчёта]:", e)
            self.show_dialog("Ошибка", "Не удалось сформировать отчёт.")
                
        self.send_email_with_report(email, report)
    ...
    def send_email_with_report(self, to_email, report_text):
        try:
            ...
            msg["From"] = "funny2006a@gmail.com"  # Заменить!
            msg["To"] = to_email
            ...
            smtp_port = 587
            your_email = "funny2006a@gmail.com"  # Заменить!
